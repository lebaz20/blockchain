...
|--------|-------|---------------|
| Total Samples | 10686 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 56 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 118.73 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 10686 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 30 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 2981 | Real (non-duplicate) transactions confirmed on-chain; duplicate injections are excluded from this count |
| Unassigned Transactions | 6414 | Real transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** |
| Avg Transactions per Block | 99.36 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 164 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 18.17 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 27.89 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 5.07 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

> **Duplicate injection:** 4817 duplicate transactions were injected by Enhanced nodes (50% probability per transaction) to simulate dual-shard verification. All TX metrics above are corrected to reflect **real transactions only** using the formula `real = raw × fired / (fired + duplicates)`. (**warning:** only 17/24 nodes responded — duplicate count may be understated)

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 13876 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 33 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 91.78 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 154.17 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 13876 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 50 | Blocks committed across all shards |
| Transactions in Blocks | 5000 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 6487 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 174 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 28.73 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 36.03 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 10.35 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 10686 | 2981 | **~27.89%** | 18.17 tx/s | **~5.07 tx/s** |
| **PBFT-RapidChain** | 13876 | 5000 | **~36.03%** | 28.73 tx/s | **~10.35 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 118.73 | 154.17 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 56 | 33 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 100.00 | 91.78 | **Enhanced** 🏆 |
| Blocks Created | 30 | 50 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 2981 | 5000 | **RapidChain** 🏆 |
| Avg TX per Block | 99.36 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 18.17 | 28.73 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 27.89 | 36.03 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 5.07 | 10.35 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 164 | 174 | _(lower = faster drain)_ |
...
- 27.89% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 33 ms average HTTP response time
- 36.03% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (10.35 vs 5.07 tx/s) after correcting for input volume
- 33 ms vs 56 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260310_031846-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260310_032315-summary.txt`
