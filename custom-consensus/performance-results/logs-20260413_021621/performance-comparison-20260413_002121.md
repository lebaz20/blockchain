...
| JMeter Threads | 440 | 440 concurrent virtual users — identical for both protocols; high enough that the ConstantThroughputTimer cap (366 req/s) is the actual bottleneck rather than thread count |
...
|--------|-------|---------------|
| Total Samples | 14243 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 1365 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 94.46 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 142.43 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 14243 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 254 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 12281 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 4360 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 48.35 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 327 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 37.55 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 86.22 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 32.38 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 20412 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 487 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 96.92 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 204.12 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 20412 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 216 | Blocks committed across all shards |
| Transactions in Blocks | 21600 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 1758 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 156 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 138.46 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 105.82 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 146.52 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 14243 | 12281 | **~86.22%** | 37.55 tx/s | **~32.38 tx/s** |
| **PBFT-RapidChain** | 20412 | 21600 | **~105.82%** | 138.46 tx/s | **~146.52 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 142.43 | 204.12 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 1365 | 487 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 94.46 | 96.92 | **RapidChain** 🏆 |
| Blocks Created | 254 | 216 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 12281 | 21600 | **RapidChain** 🏆 |
| Avg TX per Block | 48.35 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 37.55 | 138.46 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 86.22 | 105.82 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 32.38 | 146.52 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 327 | 156 | _(lower = faster drain)_ |
...
**Strengths:**
- 94.46% HTTP success rate
- 86.22% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 487 ms average HTTP response time
- 105.82% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (146.52 vs 32.38 tx/s) after correcting for input volume
- 487 ms vs 1365 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260413_002121-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260413_003020-summary.txt`
