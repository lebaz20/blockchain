...
| JMeter Threads | 110 | 110 concurrent virtual users — identical for both protocols; high enough that the ConstantThroughputTimer cap (366 req/s) is the actual bottleneck rather than thread count |
| Test Duration | 100 s | JMeter fires transactions for 100 seconds total; active load = 70 s, ramp-down = 30 s |
| Ramp-up Time | 5 s | JMeter linearly scales from 0 to 110 threads over the first 5 seconds to avoid a cold-start spike |
...
|--------|-------|---------------|
| Total Samples | 23647 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 61 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 236.47 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 23647 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 194 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 8683 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 6298 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 44.75 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 130 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 66.79 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 36.71 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 24.52 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 19895 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 123 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.55 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 198.95 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 19895 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 195 | Blocks committed across all shards |
| Transactions in Blocks | 19500 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 1104 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 137 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 142.33 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 98.01 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 139.50 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 23647 | 8683 | **~36.71%** | 66.79 tx/s | **~24.52 tx/s** |
| **PBFT-RapidChain** | 19895 | 19500 | **~98.01%** | 142.33 tx/s | **~139.50 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 236.47 | 198.95 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 61 | 123 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 99.55 | **Enhanced** 🏆 |
| Blocks Created | 194 | 195 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 8683 | 19500 | **RapidChain** 🏆 |
| Avg TX per Block | 44.75 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 66.79 | 142.33 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 36.71 | 98.01 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 24.52 | 139.50 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 130 | 137 | _(lower = faster drain)_ |
...
- 36.71% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 123 ms average HTTP response time
- 98.01% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 99.55%
- **36.71% drain rate**
- Higher Effective TX Rate (24.52 vs 139.50 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260407_230625-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260407_231859-summary.txt`
