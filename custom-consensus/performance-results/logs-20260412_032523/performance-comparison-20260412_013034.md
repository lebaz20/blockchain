...
|--------|-------|---------------|
| Total Samples | 30140 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 551 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.28 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 301.40 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 30140 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 319 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 31347 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 4324 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 98.26 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 201 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 155.95 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 104.00 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 162.20 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 34170 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 406 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.96 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 341.70 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 34170 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 237 | Blocks committed across all shards |
| Transactions in Blocks | 23700 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 4031 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 169.28 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 69.35 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 117.41 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 30140 | 31347 | **~104.00%** | 155.95 tx/s | **~162.20 tx/s** |
| **PBFT-RapidChain** | 34170 | 23700 | **~69.35%** | 169.28 tx/s | **~117.41 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 301.40 | 341.70 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 551 | 406 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 97.28 | 97.96 | **RapidChain** 🏆 |
| Blocks Created | 319 | 237 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 31347 | 23700 | **Enhanced** 🏆 |
| Avg TX per Block | 98.26 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 155.95 | 169.28 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 104.00 | 69.35 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 162.20 | 117.41 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 201 | 140 | _(lower = faster drain)_ |
...
**Strengths:**
- 97.28% HTTP success rate
- 104.00% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 406 ms average HTTP response time
- 69.35% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (117.41 vs 162.20 tx/s) after correcting for input volume
- 406 ms vs 551 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_013034-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_014533-summary.txt`
