...
|--------|-------|---------------|
| Total Samples | 11362 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 297 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 113.62 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 11362 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 218 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 10288 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
| Cross-Shard Verification TX | 4 | Transactions from other healthy shards re-validated and committed by this shard's PBFT — excluded from Transactions in Blocks, Drain Rate, and Effective TX Rate |
| Unassigned Transactions | 1074 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 47.19 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 142 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 72.45 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 90.54 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 65.60 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 6652 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 788 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 93.56 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 66.52 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 6652 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 35 | Blocks committed across all shards |
| Transactions in Blocks | 3500 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 3061 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 26.11 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 52.61 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 13.74 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 11362 | 10288 | **~90.54%** | 72.45 tx/s | **~65.60 tx/s** |
| **PBFT-RapidChain** | 6652 | 3500 | **~52.61%** | 26.11 tx/s | **~13.74 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 113.62 | 66.52 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 297 | 788 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 93.56 | **Enhanced** 🏆 |
| Blocks Created | 218 | 35 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 10288 | 3500 | **Enhanced** 🏆 |
| Avg TX per Block | 47.19 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 72.45 | 26.11 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 90.54 | 52.61 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 65.60 | 13.74 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 142 | 134 | _(lower = faster drain)_ |
...
- 90.54% drain rate (share of submitted transactions confirmed on-chain)
- Single-layer consensus (no committee stage) with lower operational complexity
- Cross-shard verification: each healthy shard re-validates the next healthy shard's committed blocks (cyclic ring assignment); 4 verification TXs committed this run
...
**Strengths:**
- 788 ms average HTTP response time
- 52.61% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 93.56%
- **90.54% drain rate**
- Higher Effective TX Rate (65.60 vs 13.74 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260414_125420-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260414_130737-summary.txt`
