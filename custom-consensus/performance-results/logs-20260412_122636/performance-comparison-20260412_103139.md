...
|--------|-------|---------------|
| Total Samples | 2213 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 855 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.23 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 22.13 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 2213 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 13 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 1226 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Avg Transactions per Block | 94.30 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 111 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 11.04 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 55.39 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 6.11 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 3141 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 454 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 31.41 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 3141 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
...
| Unassigned Transactions | 2 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 123 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 17.88 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 70.04 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 12.52 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 2213 | 1226 | **~55.39%** | 11.04 tx/s | **~6.11 tx/s** |
| **PBFT-RapidChain** | 3141 | 2200 | **~70.04%** | 17.88 tx/s | **~12.52 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 22.13 | 31.41 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 855 | 454 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 99.23 | 100.00 | **RapidChain** 🏆 |
| Blocks Created | 13 | 22 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 1226 | 2200 | **RapidChain** 🏆 |
| Avg TX per Block | 94.30 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 11.04 | 17.88 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 55.39 | 70.04 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 6.11 | 12.52 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 111 | 123 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.23% HTTP success rate
- 55.39% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 454 ms average HTTP response time
- 70.04% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (12.52 vs 6.11 tx/s) after correcting for input volume
- 454 ms vs 855 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_103140-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_103433-summary.txt`
