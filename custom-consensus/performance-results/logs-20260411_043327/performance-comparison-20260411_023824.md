...
|--------|-------|---------------|
| Total Samples | 45753 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 160 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.99 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 457.53 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 45753 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 105 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 10663 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 15173 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 101.55 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 152 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 70.15 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 23.30 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 16.34 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 40464 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 254 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.52 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 404.64 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 40464 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
...
| Unassigned Transactions | 1419 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 255.55 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 85.26 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 217.88 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 45753 | 10663 | **~23.30%** | 70.15 tx/s | **~16.34 tx/s** |
| **PBFT-RapidChain** | 40464 | 34500 | **~85.26%** | 255.55 tx/s | **~217.88 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 457.53 | 404.64 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 160 | 254 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 99.99 | 99.52 | **Enhanced** 🏆 |
| Blocks Created | 105 | 345 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 10663 | 34500 | **RapidChain** 🏆 |
| Avg TX per Block | 101.55 | 100.00 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 70.15 | 255.55 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 23.30 | 85.26 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 16.34 | 217.88 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 152 | 135 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.99% HTTP success rate
- 23.30% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 254 ms average HTTP response time
- 85.26% drain rate (share of submitted transactions confirmed on-chain)
...
- **99.99% acceptance rate** vs RapidChain's 99.52%
- **23.30% drain rate**
- Higher Effective TX Rate (16.34 vs 217.88 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260411_023824-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260411_024724-summary.txt`
