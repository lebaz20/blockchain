...
|--------|-------|---------------|
| Total Samples | 29140 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 575 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 96.16 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 291.40 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 29140 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 203 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 19049 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 4980 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 93.83 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 207 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 92.02 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 65.37 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 60.15 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 36315 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 343 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.09 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 363.15 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36315 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 273 | Blocks committed across all shards |
| Transactions in Blocks | 27300 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 3974 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 206.81 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 75.17 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 155.47 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 29140 | 19049 | **~65.37%** | 92.02 tx/s | **~60.15 tx/s** |
| **PBFT-RapidChain** | 36315 | 27300 | **~75.17%** | 206.81 tx/s | **~155.47 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 291.40 | 363.15 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 575 | 343 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 96.16 | 99.09 | **RapidChain** 🏆 |
| Blocks Created | 203 | 273 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 19049 | 27300 | **RapidChain** 🏆 |
| Avg TX per Block | 93.83 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 92.02 | 206.81 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 65.37 | 75.17 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 60.15 | 155.47 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 207 | 132 | _(lower = faster drain)_ |
...
**Strengths:**
- 96.16% HTTP success rate
- 65.37% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 343 ms average HTTP response time
- 75.17% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (155.47 vs 60.15 tx/s) after correcting for input volume
- 343 ms vs 575 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_020903-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_022352-summary.txt`
