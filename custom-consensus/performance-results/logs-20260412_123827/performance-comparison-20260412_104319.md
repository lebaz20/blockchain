...
|--------|-------|---------------|
| Total Samples | 3809 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 676 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.79 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 38.09 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 3809 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 29 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 2716 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 1170 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 93.65 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 21.55 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 71.30 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 15.37 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 2469 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 1302 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 89.95 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 24.69 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 2469 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
...
| Unassigned Transactions | 468 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 176 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 15.34 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 109.35 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 16.77 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 3809 | 2716 | **~71.30%** | 21.55 tx/s | **~15.37 tx/s** |
| **PBFT-RapidChain** | 2469 | 2700 | **~109.35%** | 15.34 tx/s | **~16.77 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 38.09 | 24.69 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 676 | 1302 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 98.79 | 89.95 | **Enhanced** 🏆 |
| Blocks Created | 29 | 27 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 2716 | 2700 | **Enhanced** 🏆 |
| Avg TX per Block | 93.65 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 21.55 | 15.34 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 71.30 | 109.35 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 15.37 | 16.77 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 126 | 176 | _(lower = faster drain)_ |
...
**Strengths:**
- 98.79% HTTP success rate
- 71.30% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 1302 ms average HTTP response time
- 109.35% drain rate (share of submitted transactions confirmed on-chain)
...
- **98.79% acceptance rate** vs RapidChain's 89.95%
- **71.30% drain rate**
- Higher Effective TX Rate (15.37 vs 16.77 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_104319-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_104651-summary.txt`
