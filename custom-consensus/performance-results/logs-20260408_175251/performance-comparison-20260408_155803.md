...
|--------|-------|---------------|
| Total Samples | 38604 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 296 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.93 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 386.04 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 38604 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 240 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 12455 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 10602 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 51.89 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 153 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 81.40 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 32.26 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 26.26 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 36783 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 342 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.91 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 367.83 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36783 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 305 | Blocks committed across all shards |
| Transactions in Blocks | 30500 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 3172 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 163 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 187.11 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 82.91 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 155.15 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 38604 | 12455 | **~32.26%** | 81.40 tx/s | **~26.26 tx/s** |
| **PBFT-RapidChain** | 36783 | 30500 | **~82.91%** | 187.11 tx/s | **~155.15 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 386.04 | 367.83 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 296 | 342 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 99.93 | 98.91 | **Enhanced** 🏆 |
| Blocks Created | 240 | 305 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 12455 | 30500 | **RapidChain** 🏆 |
| Avg TX per Block | 51.89 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 81.40 | 187.11 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 32.26 | 82.91 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 26.26 | 155.15 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 153 | 163 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.93% HTTP success rate
- 32.26% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 342 ms average HTTP response time
- 82.91% drain rate (share of submitted transactions confirmed on-chain)
...
- **99.93% acceptance rate** vs RapidChain's 98.91%
- **32.26% drain rate**
- Higher Effective TX Rate (26.26 vs 155.15 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260408_155803-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260408_161222-summary.txt`
