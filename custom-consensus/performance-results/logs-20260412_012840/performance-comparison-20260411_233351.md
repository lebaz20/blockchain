...
|--------|-------|---------------|
| Total Samples | 36713 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 347 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.71 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 367.13 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36713 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 394 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 38480 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 2596 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 97.66 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 221 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 174.11 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 104.81 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 182.49 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 34344 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 404 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.94 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 343.44 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 34344 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 316 | Blocks committed across all shards |
| Transactions in Blocks | 31600 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 2776 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 232.35 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 92.01 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 213.78 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 36713 | 38480 | **~104.81%** | 174.11 tx/s | **~182.49 tx/s** |
| **PBFT-RapidChain** | 34344 | 31600 | **~92.01%** | 232.35 tx/s | **~213.78 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 367.13 | 343.44 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 347 | 404 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 98.71 | 98.94 | **RapidChain** 🏆 |
| Blocks Created | 394 | 316 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 38480 | 31600 | **Enhanced** 🏆 |
| Avg TX per Block | 97.66 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 174.11 | 232.35 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 104.81 | 92.01 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 182.49 | 213.78 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 221 | 136 | _(lower = faster drain)_ |
...
**Strengths:**
- 98.71% HTTP success rate
- 104.81% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 404 ms average HTTP response time
- 92.01% drain rate (share of submitted transactions confirmed on-chain)
...
- **98.71% acceptance rate** vs RapidChain's 98.94%
- **104.81% drain rate**
- Higher Effective TX Rate (182.49 vs 213.78 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260411_233351-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260411_234913-summary.txt`
