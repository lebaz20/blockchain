...
|--------|-------|---------------|
| Total Samples | 17273 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 492 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 172.73 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 17273 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 103 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 9669 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 7069 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 93.87 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 151 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 64.03 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 55.97 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 35.84 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 13999 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 648 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.79 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 139.99 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 13999 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 159 | Blocks committed across all shards |
| Transactions in Blocks | 14651 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 12996 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 92.14 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 142 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 103.17 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 104.65 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 107.98 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 17273 | 9669 | **~55.97%** | 64.03 tx/s | **~35.84 tx/s** |
| **PBFT-RapidChain** | 13999 | 14651 | **~104.65%** | 103.17 tx/s | **~107.98 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 172.73 | 139.99 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 492 | 648 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 97.79 | **Enhanced** 🏆 |
| Blocks Created | 103 | 159 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 9669 | 14651 | **RapidChain** 🏆 |
| Avg TX per Block | 93.87 | 92.14 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 64.03 | 103.17 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 55.97 | 104.65 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 35.84 | 107.98 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 151 | 142 | _(lower = faster drain)_ |
...
- 55.97% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 648 ms average HTTP response time
- 104.65% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 97.79%
- **55.97% drain rate**
- Higher Effective TX Rate (35.84 vs 107.98 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260413_193058-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260413_194424-summary.txt`
