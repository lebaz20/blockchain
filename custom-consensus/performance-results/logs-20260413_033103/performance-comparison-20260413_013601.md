...
|--------|-------|---------------|
| Total Samples | 13888 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 688 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.97 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 138.88 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 13888 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 145 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 14325 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 1079 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 98.79 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 322 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 44.48 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 103.14 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 45.88 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 17867 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 363 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.32 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 178.67 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 17867 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 177 | Blocks committed across all shards |
| Transactions in Blocks | 17700 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 1320 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 155 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 114.19 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 99.06 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 113.12 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 13888 | 14325 | **~103.14%** | 44.48 tx/s | **~45.88 tx/s** |
| **PBFT-RapidChain** | 17867 | 17700 | **~99.06%** | 114.19 tx/s | **~113.12 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 138.88 | 178.67 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 688 | 363 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 97.97 | 98.32 | **RapidChain** 🏆 |
| Blocks Created | 145 | 177 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 14325 | 17700 | **RapidChain** 🏆 |
| Avg TX per Block | 98.79 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 44.48 | 114.19 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 103.14 | 99.06 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 45.88 | 113.12 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 322 | 155 | _(lower = faster drain)_ |
...
**Strengths:**
- 97.97% HTTP success rate
- 103.14% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 363 ms average HTTP response time
- 99.06% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (113.12 vs 45.88 tx/s) after correcting for input volume
- 363 ms vs 688 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260413_013601-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260413_014944-summary.txt`
