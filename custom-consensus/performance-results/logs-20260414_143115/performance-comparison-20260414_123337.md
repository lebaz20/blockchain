...
**Test Date:** 2026-04-14 12:49:21

## Test Configuration

| Parameter | Value | Meaning |
|-----------|-------|---------|
| Number of Nodes (Enhanced) | 32 | 32 P2P nodes ran the PBFT-Enhanced consensus; each participates in every round of consensus voting |
| Number of Nodes (RapidChain) | 32 total | Nodes split into shards of 4, plus 1 committee shard for cross-shard finality |
| Faulty Nodes (RapidChain) | 10 | Byzantine-simulated nodes that do not propose or vote in consensus |
| JMeter Threads | 60 | 60 concurrent virtual users — identical for both protocols; high enough that the ConstantThroughputTimer cap (100 req/s) is the actual bottleneck rather than thread count |
| Test Duration | 100 s | JMeter fires transactions for 100 seconds total; active load = 70 s, ramp-down = 30 s |
| Ramp-up Time | 5 s | JMeter linearly scales from 0 to 60 threads over the first 5 seconds to avoid a cold-start spike |
...
|--------|-------|---------------|
| Total Samples | 4972 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 456 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 49.72 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 4972 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 120 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 4892 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
| Cross-Shard Verification TX | 54 | Transactions from other healthy shards re-validated and committed by this shard's PBFT — excluded from Transactions in Blocks, Drain Rate, and Effective TX Rate |
...
| Avg Transactions per Block | 40.76 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 33.27 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 98.39 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 32.74 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 2715 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 1141 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 91.04 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 27.15 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 2715 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 13 | Blocks committed across all shards |
| Transactions in Blocks | 1300 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 1385 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 133 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 9.77 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 47.88 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 4.68 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 4972 | 4892 | **~98.39%** | 33.27 tx/s | **~32.74 tx/s** |
| **PBFT-RapidChain** | 2715 | 1300 | **~47.88%** | 9.77 tx/s | **~4.68 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 49.72 | 27.15 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 456 | 1141 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 91.04 | **Enhanced** 🏆 |
| Blocks Created | 120 | 13 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 4892 | 1300 | **Enhanced** 🏆 |
| Avg TX per Block | 40.76 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 33.27 | 9.77 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 98.39 | 47.88 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 32.74 | 4.68 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 147 | 133 | _(lower = faster drain)_ |
...
- 98.39% drain rate (share of submitted transactions confirmed on-chain)
- Single-layer consensus (no committee stage) with lower operational complexity
- Cross-shard verification: each healthy shard re-validates the next healthy shard's committed blocks (cyclic ring assignment); 54 verification TXs committed this run
...
**Strengths:**
- 1141 ms average HTTP response time
- 47.88% drain rate (share of submitted transactions confirmed on-chain)
...
For 32 nodes, PBFT-Enhanced performs better overall:
- **100.00% acceptance rate** vs RapidChain's 91.04%
- **98.39% drain rate**
- Higher Effective TX Rate (32.74 vs 4.68 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260414_123338-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260414_124636-summary.txt`
