...
|--------|-------|---------------|
| Total Samples | 87185 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 149 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.83 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 871.85 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 87185 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 316 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 63588 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 5428 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 201.22 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 432.57 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 72.93 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 315.49 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 92703 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 100 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 927.03 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 92703 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 196 | Blocks committed across all shards |
| Transactions in Blocks | 19600 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 6946 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 151 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 129.80 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 21.14 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 27.44 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 87185 | 63588 | **~72.93%** | 432.57 tx/s | **~315.49 tx/s** |
| **PBFT-RapidChain** | 92703 | 19600 | **~21.14%** | 129.80 tx/s | **~27.44 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 871.85 | 927.03 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 149 | 100 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 99.83 | 100.00 | **RapidChain** 🏆 |
| Blocks Created | 316 | 196 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 63588 | 19600 | **Enhanced** 🏆 |
| Avg TX per Block | 201.22 | 100.00 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 432.57 | 129.80 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 72.93 | 21.14 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 315.49 | 27.44 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 147 | 151 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.83% HTTP success rate
- 72.93% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 100 ms average HTTP response time
- 21.14% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (27.44 vs 315.49 tx/s) after correcting for input volume
- 100 ms vs 149 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260407_211402-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260407_213126-summary.txt`
