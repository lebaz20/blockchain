...
| JMeter Threads | 430 | 430 concurrent virtual users — identical for both protocols; high enough that the ConstantThroughputTimer cap (1433 req/s) is the actual bottleneck rather than thread count |
| Test Duration | 100 s | JMeter fires transactions for 100 seconds total; active load = 70 s, ramp-down = 30 s |
| Ramp-up Time | 5 s | JMeter linearly scales from 0 to 430 threads over the first 5 seconds to avoid a cold-start spike |
...
|--------|-------|---------------|
| Total Samples | 50979 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 355 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 509.79 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 50979 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 331 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 66448 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 9049 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 200.74 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 159 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 417.91 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 130.34 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 544.72 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 87087 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 84 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 870.87 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 87087 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 286 | Blocks committed across all shards |
| Transactions in Blocks | 28600 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 9121 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 161 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 177.63 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 32.84 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 58.33 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 50979 | 66448 | **~130.34%** | 417.91 tx/s | **~544.72 tx/s** |
| **PBFT-RapidChain** | 87087 | 28600 | **~32.84%** | 177.63 tx/s | **~58.33 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 509.79 | 870.87 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 355 | 84 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 99.00 | 100.00 | **RapidChain** 🏆 |
| Blocks Created | 331 | 286 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 66448 | 28600 | **Enhanced** 🏆 |
| Avg TX per Block | 200.74 | 100.00 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 417.91 | 177.63 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 130.34 | 32.84 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 544.72 | 58.33 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 159 | 161 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.00% HTTP success rate
- 130.34% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 84 ms average HTTP response time
...
- Higher Effective TX Rate (58.33 vs 544.72 tx/s) after correcting for input volume
- 84 ms vs 355 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260407_162731-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260407_164522-summary.txt`
