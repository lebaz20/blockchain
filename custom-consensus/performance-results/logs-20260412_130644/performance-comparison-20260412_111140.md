...
|--------|-------|---------------|
| Total Samples | 14450 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 608 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.46 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 144.50 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 14450 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 128 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 12405 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 2283 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.91 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 188 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 65.98 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 85.84 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 56.64 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 16100 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 454 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 96.93 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 161.00 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 16100 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 165 | Blocks committed across all shards |
| Transactions in Blocks | 16500 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 1154 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 123.13 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 102.48 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 126.19 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 14450 | 12405 | **~85.84%** | 65.98 tx/s | **~56.64 tx/s** |
| **PBFT-RapidChain** | 16100 | 16500 | **~102.48%** | 123.13 tx/s | **~126.19 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 144.50 | 161.00 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 608 | 454 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 97.46 | 96.93 | **Enhanced** 🏆 |
| Blocks Created | 128 | 165 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 12405 | 16500 | **RapidChain** 🏆 |
| Avg TX per Block | 96.91 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 65.98 | 123.13 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 85.84 | 102.48 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 56.64 | 126.19 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 188 | 134 | _(lower = faster drain)_ |
...
**Strengths:**
- 97.46% HTTP success rate
- 85.84% drain rate (share of submitted transactions confirmed on-chain)
...
- 102.48% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (126.19 vs 56.64 tx/s) after correcting for input volume
- 454 ms vs 608 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_111140-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_112317-summary.txt`
