...
|--------|-------|---------------|
| Total Samples | 23521 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 164 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 235.21 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 23521 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 134 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 12600 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 9879 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 94.02 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 314 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 40.12 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 53.56 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 21.49 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 15670 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 459 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 96.18 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 156.70 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 15670 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 85 | Blocks committed across all shards |
| Transactions in Blocks | 8500 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 7169 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 62.50 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 54.24 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 33.90 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 23521 | 12600 | **~53.56%** | 40.12 tx/s | **~21.49 tx/s** |
| **PBFT-RapidChain** | 15670 | 8500 | **~54.24%** | 62.50 tx/s | **~33.90 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 235.21 | 156.70 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 164 | 459 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 96.18 | **Enhanced** 🏆 |
| Blocks Created | 134 | 85 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 12600 | 8500 | **Enhanced** 🏆 |
| Avg TX per Block | 94.02 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 40.12 | 62.50 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 53.56 | 54.24 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 21.49 | 33.90 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 314 | 136 | _(lower = faster drain)_ |
...
- 53.56% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 459 ms average HTTP response time
- 54.24% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 96.18%
- **53.56% drain rate**
- Higher Effective TX Rate (21.49 vs 33.90 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260414_025644-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260414_031338-summary.txt`
