...
|--------|-------|---------------|
| Total Samples | 45381 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 168 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 453.81 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 45381 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 141 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 19371 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 9218 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 137.38 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 126.60 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 42.68 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 54.04 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 36227 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 358 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.28 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 362.27 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36227 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 327 | Blocks committed across all shards |
| Transactions in Blocks | 32700 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 2093 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 210.96 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 90.26 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 190.42 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 45381 | 19371 | **~42.68%** | 126.60 tx/s | **~54.04 tx/s** |
| **PBFT-RapidChain** | 36227 | 32700 | **~90.26%** | 210.96 tx/s | **~190.42 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 453.81 | 362.27 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 168 | 358 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 98.28 | **Enhanced** 🏆 |
| Blocks Created | 141 | 327 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 19371 | 32700 | **RapidChain** 🏆 |
| Avg TX per Block | 137.38 | 100.00 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 126.60 | 210.96 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 42.68 | 90.26 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 54.04 | 190.42 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 153 | 155 | _(lower = faster drain)_ |
...
- 42.68% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 358 ms average HTTP response time
- 90.26% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 98.28%
- **42.68% drain rate**
- Higher Effective TX Rate (54.04 vs 190.42 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260411_032740-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260411_034154-summary.txt`
