...
|--------|-------|---------------|
| Total Samples | 27962 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 660 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.65 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 279.62 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 27962 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 298 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 28847 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 1027 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.80 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 181 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 159.37 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 103.16 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 164.41 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 35920 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 361 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.08 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 359.20 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 35920 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 337 | Blocks committed across all shards |
| Transactions in Blocks | 33700 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 3010 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 149 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 226.17 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 93.81 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 212.19 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 27962 | 28847 | **~103.16%** | 159.37 tx/s | **~164.41 tx/s** |
| **PBFT-RapidChain** | 35920 | 33700 | **~93.81%** | 226.17 tx/s | **~212.19 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 279.62 | 359.20 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 660 | 361 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 97.65 | 98.08 | **RapidChain** 🏆 |
| Blocks Created | 298 | 337 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 28847 | 33700 | **RapidChain** 🏆 |
| Avg TX per Block | 96.80 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 159.37 | 226.17 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 103.16 | 93.81 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 164.41 | 212.19 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 181 | 149 | _(lower = faster drain)_ |
...
**Strengths:**
- 97.65% HTTP success rate
- 103.16% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 361 ms average HTTP response time
- 93.81% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (212.19 vs 164.41 tx/s) after correcting for input volume
- 361 ms vs 660 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_113502-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_114946-summary.txt`
