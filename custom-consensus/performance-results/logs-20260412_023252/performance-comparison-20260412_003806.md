...
|--------|-------|---------------|
| Total Samples | 33027 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 435 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.45 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 330.27 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 33027 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 416 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 39909 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 1862 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 95.93 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 275 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 145.12 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 120.83 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 175.36 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 39860 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 279 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.73 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 398.60 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 39860 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 364 | Blocks committed across all shards |
| Transactions in Blocks | 36400 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 2697 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 153 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 237.90 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 91.31 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 217.25 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 33027 | 39909 | **~120.83%** | 145.12 tx/s | **~175.36 tx/s** |
| **PBFT-RapidChain** | 39860 | 36400 | **~91.31%** | 237.90 tx/s | **~217.25 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 330.27 | 398.60 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 435 | 279 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 97.45 | 99.73 | **RapidChain** 🏆 |
| Blocks Created | 416 | 364 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 39909 | 36400 | **Enhanced** 🏆 |
| Avg TX per Block | 95.93 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 145.12 | 237.90 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 120.83 | 91.31 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 175.36 | 217.25 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 275 | 153 | _(lower = faster drain)_ |
...
**Strengths:**
- 97.45% HTTP success rate
- 120.83% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 279 ms average HTTP response time
- 91.31% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (217.25 vs 175.36 tx/s) after correcting for input volume
- 279 ms vs 435 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_003807-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_005417-summary.txt`
