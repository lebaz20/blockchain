...
|--------|-------|---------------|
| Total Samples | 39477 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 280 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.85 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 394.77 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 39477 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 256 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 10913 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 11636 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 42.62 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 71.32 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 27.64 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 19.71 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 36174 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 356 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.71 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 361.74 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36174 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 356 | Blocks committed across all shards |
| Transactions in Blocks | 35600 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 2539 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 254.28 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 98.41 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 250.25 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 39477 | 10913 | **~27.64%** | 71.32 tx/s | **~19.71 tx/s** |
| **PBFT-RapidChain** | 36174 | 35600 | **~98.41%** | 254.28 tx/s | **~250.25 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 394.77 | 361.74 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 280 | 356 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 99.85 | 98.71 | **Enhanced** 🏆 |
| Blocks Created | 256 | 356 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 10913 | 35600 | **RapidChain** 🏆 |
| Avg TX per Block | 42.62 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 71.32 | 254.28 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 27.64 | 98.41 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 19.71 | 250.25 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 153 | 140 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.85% HTTP success rate
- 27.64% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 356 ms average HTTP response time
- 98.41% drain rate (share of submitted transactions confirmed on-chain)
...
- **99.85% acceptance rate** vs RapidChain's 98.71%
- **27.64% drain rate**
- Higher Effective TX Rate (19.71 vs 250.25 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260408_141925-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260408_143332-summary.txt`
