...
|--------|-------|---------------|
| Total Samples | 13877 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 667 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.29 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 138.77 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 13877 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 152 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 14718 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 98 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.82 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 119 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 123.68 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 106.06 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 131.17 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 17830 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 369 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.54 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 178.30 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 17830 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 189 | Blocks committed across all shards |
| Transactions in Blocks | 18900 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 649 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 138.97 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 106.00 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 147.31 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 13877 | 14718 | **~106.06%** | 123.68 tx/s | **~131.17 tx/s** |
| **PBFT-RapidChain** | 17830 | 18900 | **~106.00%** | 138.97 tx/s | **~147.31 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 138.77 | 178.30 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 667 | 369 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 98.29 | 97.54 | **Enhanced** 🏆 |
| Blocks Created | 152 | 189 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 14718 | 18900 | **RapidChain** 🏆 |
| Avg TX per Block | 96.82 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 123.68 | 138.97 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 106.06 | 106.00 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 131.17 | 147.31 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 119 | 136 | _(lower = faster drain)_ |
...
**Strengths:**
- 98.29% HTTP success rate
- 106.06% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 369 ms average HTTP response time
- 106.00% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (147.31 vs 131.17 tx/s) after correcting for input volume
- 369 ms vs 667 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_224842-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_225916-summary.txt`
