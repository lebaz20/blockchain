...
|--------|-------|---------------|
| Total Samples | 43338 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 211 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 433.38 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 43338 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 229 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 22043 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 448 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.25 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 138.63 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 50.86 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 70.51 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 36608 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 344 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.11 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 366.08 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36608 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 293 | Blocks committed across all shards |
| Transactions in Blocks | 29300 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 2838 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 209.28 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 80.03 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 167.50 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 43338 | 22043 | **~50.86%** | 138.63 tx/s | **~70.51 tx/s** |
| **PBFT-RapidChain** | 36608 | 29300 | **~80.03%** | 209.28 tx/s | **~167.50 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 433.38 | 366.08 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 211 | 344 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 98.11 | **Enhanced** 🏆 |
| Blocks Created | 229 | 293 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 22043 | 29300 | **RapidChain** 🏆 |
| Avg TX per Block | 96.25 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 138.63 | 209.28 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 50.86 | 80.03 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 70.51 | 167.50 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 159 | 140 | _(lower = faster drain)_ |
...
- 50.86% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 344 ms average HTTP response time
- 80.03% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 98.11%
- **50.86% drain rate**
- Higher Effective TX Rate (70.51 vs 167.50 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260411_153704-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260411_155123-summary.txt`
