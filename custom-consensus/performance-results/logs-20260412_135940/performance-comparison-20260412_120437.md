...
|--------|-------|---------------|
| Total Samples | 50140 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 786 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 97.86 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 501.40 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 50140 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 556 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 53930 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 760 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.99 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 297.95 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 107.55 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 320.47 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 94909 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 82 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.98 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 949.09 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 94909 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 389 | Blocks committed across all shards |
| Transactions in Blocks | 38900 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 4609 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 222.28 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 40.98 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 91.10 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 50140 | 53930 | **~107.55%** | 297.95 tx/s | **~320.47 tx/s** |
| **PBFT-RapidChain** | 94909 | 38900 | **~40.98%** | 222.28 tx/s | **~91.10 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 501.40 | 949.09 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 786 | 82 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 97.86 | 99.98 | **RapidChain** 🏆 |
| Blocks Created | 556 | 389 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 53930 | 38900 | **Enhanced** 🏆 |
| Avg TX per Block | 96.99 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 297.95 | 222.28 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 107.55 | 40.98 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 320.47 | 91.10 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 181 | 175 | _(lower = faster drain)_ |
...
**Strengths:**
- 97.86% HTTP success rate
- 107.55% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 82 ms average HTTP response time
- 40.98% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (91.10 vs 320.47 tx/s) after correcting for input volume
- 82 ms vs 786 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_120437-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_122539-summary.txt`
