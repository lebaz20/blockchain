...
|--------|-------|---------------|
| Total Samples | 43021 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 234 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 430.21 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 43021 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 429 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 39673 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 2845 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 92.47 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 259.30 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 92.21 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 239.12 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 39611 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 243 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.83 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 396.11 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 39611 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 174 | Blocks committed across all shards |
| Transactions in Blocks | 17400 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 22907 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 116.00 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 43.92 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 50.95 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 43021 | 39673 | **~92.21%** | 259.30 tx/s | **~239.12 tx/s** |
| **PBFT-RapidChain** | 39611 | 17400 | **~43.92%** | 116.00 tx/s | **~50.95 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 430.21 | 396.11 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 234 | 243 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 100.00 | 98.83 | **Enhanced** 🏆 |
| Blocks Created | 429 | 174 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 39673 | 17400 | **Enhanced** 🏆 |
| Avg TX per Block | 92.47 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 259.30 | 116.00 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 92.21 | 43.92 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 239.12 | 50.95 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 153 | 150 | _(lower = faster drain)_ |
...
- 92.21% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 243 ms average HTTP response time
- 43.92% drain rate (share of submitted transactions confirmed on-chain)
...
- **100.00% acceptance rate** vs RapidChain's 98.83%
- **92.21% drain rate**
- Higher Effective TX Rate (239.12 vs 50.95 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260414_132639-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260414_134226-summary.txt`
