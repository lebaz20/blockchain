...
|--------|-------|---------------|
| Total Samples | 20521 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 290 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 205.21 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 20521 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 108 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 10233 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Avg Transactions per Block | 94.75 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
...
| Blockchain TX Rate (tx/s) | 70.57 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 49.86 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 35.19 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 23598 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 176 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 235.98 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 23598 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 185 | Blocks committed across all shards |
| Transactions in Blocks | 17346 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 27098 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 93.76 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 128 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 135.51 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 73.50 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 99.61 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 20521 | 10233 | **~49.86%** | 70.57 tx/s | **~35.19 tx/s** |
| **PBFT-RapidChain** | 23598 | 17346 | **~73.50%** | 135.51 tx/s | **~99.61 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 205.21 | 235.98 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 290 | 176 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 100.00 | 100.00 | **RapidChain** 🏆 |
| Blocks Created | 108 | 185 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 10233 | 17346 | **RapidChain** 🏆 |
| Avg TX per Block | 94.75 | 93.76 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 70.57 | 135.51 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 49.86 | 73.50 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 35.19 | 99.61 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 145 | 128 | _(lower = faster drain)_ |
...
- 49.86% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 176 ms average HTTP response time
- 73.50% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (99.61 vs 35.19 tx/s) after correcting for input volume
- 176 ms vs 290 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260413_223812-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260413_225131-summary.txt`
