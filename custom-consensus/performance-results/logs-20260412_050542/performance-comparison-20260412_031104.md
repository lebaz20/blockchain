...
|--------|-------|---------------|
| Total Samples | 28634 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 631 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.26 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 286.34 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 28634 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 311 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 30031 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 753 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.56 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 160 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 187.69 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 104.87 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 196.85 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 35914 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 366 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.26 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 359.14 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 35914 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 287 | Blocks committed across all shards |
| Transactions in Blocks | 28700 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 4864 | Transactions still in shard memory pools at the end — **never confirmed** |
...
| Blockchain TX Rate (tx/s) | 205.00 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 79.91 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 163.82 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 28634 | 30031 | **~104.87%** | 187.69 tx/s | **~196.85 tx/s** |
| **PBFT-RapidChain** | 35914 | 28700 | **~79.91%** | 205.00 tx/s | **~163.82 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 286.34 | 359.14 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 631 | 366 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 98.26 | 98.26 | **RapidChain** 🏆 |
| Blocks Created | 311 | 287 | **Enhanced** 🏆 |
| Transactions in Blocks ⁴ | 30031 | 28700 | **Enhanced** 🏆 |
| Avg TX per Block | 96.56 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 187.69 | 205.00 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 104.87 | 79.91 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 196.85 | 163.82 | **Enhanced** 🏆 |
| Total Test Elapsed (s) ⁸ | 160 | 140 | _(lower = faster drain)_ |
...
**Strengths:**
- 98.26% HTTP success rate
- 104.87% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 366 ms average HTTP response time
- 79.91% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (163.82 vs 196.85 tx/s) after correcting for input volume
- 366 ms vs 631 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260412_031104-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260412_032525-summary.txt`
