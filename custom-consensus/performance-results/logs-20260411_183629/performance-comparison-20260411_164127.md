...
|--------|-------|---------------|
| Total Samples | 27538 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 666 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 96.51 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 275.38 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 27538 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 287 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 27818 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 1083 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 96.92 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 241 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 115.42 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 101.01 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 116.60 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 36796 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 345 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.54 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 367.96 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 36796 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 299 | Blocks committed across all shards |
| Transactions in Blocks | 29900 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 2470 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 135 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 221.48 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 81.25 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 179.97 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 27538 | 27818 | **~101.01%** | 115.42 tx/s | **~116.60 tx/s** |
| **PBFT-RapidChain** | 36796 | 29900 | **~81.25%** | 221.48 tx/s | **~179.97 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 275.38 | 367.96 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 666 | 345 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 96.51 | 98.54 | **RapidChain** 🏆 |
| Blocks Created | 287 | 299 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 27818 | 29900 | **RapidChain** 🏆 |
| Avg TX per Block | 96.92 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 115.42 | 221.48 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 101.01 | 81.25 | **Enhanced** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 116.60 | 179.97 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 241 | 135 | _(lower = faster drain)_ |
...
**Strengths:**
- 96.51% HTTP success rate
- 101.01% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 345 ms average HTTP response time
- 81.25% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (179.97 vs 116.60 tx/s) after correcting for input volume
- 345 ms vs 666 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260411_164127-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260411_165150-summary.txt`
