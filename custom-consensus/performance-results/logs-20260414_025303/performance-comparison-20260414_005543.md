...
|--------|-------|---------------|
| Total Samples | 23101 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 154 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 231.01 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 23101 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 134 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 12779 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Avg Transactions per Block | 95.36 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 146 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 87.52 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 55.31 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 48.41 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 25390 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 67 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 100.00 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 253.90 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 25390 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 221 | Blocks committed across all shards |
| Transactions in Blocks | 20988 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 27687 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 94.96 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 127 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 165.25 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 82.66 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 136.60 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 23101 | 12779 | **~55.31%** | 87.52 tx/s | **~48.41 tx/s** |
| **PBFT-RapidChain** | 25390 | 20988 | **~82.66%** | 165.25 tx/s | **~136.60 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 231.01 | 253.90 | **RapidChain** 🏆 |
| Avg Response Time (ms) ² | 154 | 67 | **RapidChain** 🏆 |
| Success Rate (%) ³ | 100.00 | 100.00 | **RapidChain** 🏆 |
| Blocks Created | 134 | 221 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 12779 | 20988 | **RapidChain** 🏆 |
| Avg TX per Block | 95.36 | 94.96 | **Enhanced** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 87.52 | 165.25 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 55.31 | 82.66 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 48.41 | 136.60 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 146 | 127 | _(lower = faster drain)_ |
...
- 55.31% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 67 ms average HTTP response time
- 82.66% drain rate (share of submitted transactions confirmed on-chain)
...
- Higher Effective TX Rate (136.60 vs 48.41 tx/s) after correcting for input volume
- 67 ms vs 154 ms average response time
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260414_005543-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260414_010902-summary.txt`
