...
|--------|-------|---------------|
| Total Samples | 39259 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 305 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 99.80 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 392.59 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 39259 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 93 | Blocks appended to the blockchain during the entire test + drain window |
| Transactions in Blocks | 5316 | Normal client transactions confirmed on-chain; counted per-tx so verification TXs mixed into the same block are still excluded |
...
| Unassigned Transactions | 11816 | Transactions still waiting in the memory pool when the drain timeout expired — **never confirmed** (normal + verification combined) |
...
| Avg Transactions per Block | 57.16 | `Transactions in Blocks ÷ Total Blocks Created` — computed on real transactions only |
| Total Test Elapsed (s) | 162 | Wall-clock seconds from test start until the pool drained to 0 (or stalled) — includes JMeter run + drain wait |
| Blockchain TX Rate (tx/s) | 32.81 | `Real Transactions in Blocks ÷ Total Test Elapsed` |
| Drain Rate (%) | 13.54 | `Real Transactions in Blocks ÷ Transactions Fired × 100` |
| Effective TX Rate (tx/s) | 4.44 | `Blockchain TX Rate × Drain Fraction` = `TX²  ÷ (Fired × Elapsed)` — penalises leaving txs unconfirmed |

### PBFT-RapidChain (Committee-Based)

| Metric | Value | What it means |
|--------|-------|---------------|
| Total Samples | 35393 | Total HTTP requests JMeter sent and received a response for (all endpoints combined: `/transaction`, `/stats`, etc.) |
| Average Response Time (ms) | 384 | Mean round-trip time for a single HTTP request from JMeter to the node and back |
| Success Rate (%) | 98.57 | Percentage of HTTP responses that returned a 2xx status |
| Throughput (req/s) | 353.93 | HTTP requests per second handled by the node API layer (JMeter perspective, all endpoints) |
| Transactions Fired by Test | 35393 | Number of those samples that were `POST /transaction` — the actual blockchain workload submitted |
| Total Blocks Created | 393 | Blocks committed across all shards |
| Transactions in Blocks | 39300 | Unique transactions confirmed on-chain across all shards |
| Unassigned Transactions | 4102 | Transactions still in shard memory pools at the end — **never confirmed** |
| Avg Transactions per Block | 100.00 | `Transactions in Blocks ÷ Total Blocks Created` |
| Total Test Elapsed (s) | 150 | Wall-clock seconds from test start until drain stalled |
| Blockchain TX Rate (tx/s) | 262.00 | `Transactions in Blocks ÷ Total Test Elapsed` — raw number can be **misleading** (see below) |
| Drain Rate (%) | 111.03 | `Transactions in Blocks ÷ Transactions Fired × 100` — how much of what was submitted actually got confirmed |
| Effective TX Rate (tx/s) | 290.92 | `TX² ÷ (Fired × Elapsed)` — corrects for input volume differences between the two runs |
...
|---|---|---|---|---|---|
| **PBFT-Enhanced** | 39259 | 5316 | **~13.54%** | 32.81 tx/s | **~4.44 tx/s** |
| **PBFT-RapidChain** | 35393 | 39300 | **~111.03%** | 262.00 tx/s | **~290.92 tx/s** |
...
|--------|---------------|-----------------|--------|
| Throughput (req/s) ¹ | 392.59 | 353.93 | **Enhanced** 🏆 |
| Avg Response Time (ms) ² | 305 | 384 | **Enhanced** 🏆 |
| Success Rate (%) ³ | 99.80 | 98.57 | **Enhanced** 🏆 |
| Blocks Created | 93 | 393 | **RapidChain** 🏆 |
| Transactions in Blocks ⁴ | 5316 | 39300 | **RapidChain** 🏆 |
| Avg TX per Block | 57.16 | 100.00 | **RapidChain** 🏆 |
| Blockchain TX Rate (tx/s) ⁵ | 32.81 | 262.00 | _(see Effective TX Rate)_ |
| Drain Rate (%) ⁶ | 13.54 | 111.03 | **RapidChain** 🏆 |
| Effective TX Rate (tx/s) ⁷ | 4.44 | 290.92 | **RapidChain** 🏆 |
| Total Test Elapsed (s) ⁸ | 162 | 150 | _(lower = faster drain)_ |
...
**Strengths:**
- 99.80% HTTP success rate
- 13.54% drain rate (share of submitted transactions confirmed on-chain)
...
**Strengths:**
- 384 ms average HTTP response time
- 111.03% drain rate (share of submitted transactions confirmed on-chain)
...
- **99.80% acceptance rate** vs RapidChain's 98.57%
- **13.54% drain rate**
- Higher Effective TX Rate (4.44 vs 290.92 tx/s) after correcting for input volume
...
- **PBFT-Enhanced Summary:** `pbft-enhanced/performance-results/pbft-enhanced-20260411_013837-summary.txt`
- **PBFT-RapidChain Summary:** `pbft-rapidchain/performance-results/pbft-rapidchain-20260411_015305-summary.txt`
