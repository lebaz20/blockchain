...
class IDAGossip {
  ...
  getSocketGossipPeers(sendersSubset, socketsKey) {
    const sockets = socketsKey ? this.socketGossipPeers[socketsKey] ?? this.socketGossipPeers : this.socketGossipPeers;
    return Object.keys(sockets).filter((port) => !sendersSubset.includes(port)).map((port) => sockets[port].socket);
  }

  getSocketGossipCore(socketsKey) {
    return socketsKey ? this.socketGossipCore[socketsKey] ?? this.socketGossipCore : this.socketGossipCore;
  }
