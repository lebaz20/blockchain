...
// Constants for transaction pool management
const TRANSACTION_REASSIGNMENT_TIMEOUT_MS = 2 * 60 * 1000 // 2 minutes
...
class TransactionPool {
  ...
  addTransaction(transaction) {
    if (!transaction || !transaction.id) {
      throw new Error(
        'Invalid transaction: transaction and transaction.id are required'
      )
    }
  ...
  assignTransactions(block) {
    if (!block || !block.hash || !Array.isArray(block.data)) {
      throw new Error(
        'Invalid block: block with hash and data array is required'
      )
    }
    ...
    }, TRANSACTION_REASSIGNMENT_TIMEOUT_MS)
  }
