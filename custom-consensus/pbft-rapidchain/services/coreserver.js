...
class Coreserver {
  ...
  listen() {
    ...
    server.on("connection", (socket, request) => {
      ...
      const subsetIndex = isCommittee ? COMMITTEE_SUBSET_INDEX : parsedUrl.searchParams.get("subsetIndex");
    ...
    socket.on("message", async (message) => {
      ...
      switch (data.type) {
        case MESSAGE_TYPE.block_to_core:
          ...
          if (
            !this.blockchain.existingBlock(data.block.hash, data.subsetIndex, isCommittee)
          ) {
            ...
            if (!isCommittee) {
              ...
              data.block.data.forEach((transaction) => {
                console.log(`COMMITTEE BLOCK TRANSACTION:`, JSON.stringify(transaction));
                this.broadcastBlock(this.blockchain.committeeChain[transaction.hash], data.subsetIndex);
              });
            }
          }
          break;
